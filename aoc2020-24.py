...
def dayflip(b, size):
    ...
    for x in range(xmin, xmax+1):
        for y in range(ymin, ymax+1):
            ...
            if (x,y) in b and (baround == 0 or baround > 2):
                towhite.append((x,y))
            elif (x,y) not in b and baround == 2:
                toblack.append((x,y))
    for tile in towhite:
        b.remove(tile)
    for tile in toblack:
...
def parttwo():
    black = set(partone())
    def size(flipped):
        s = [(min(flipped, key=lambda x:x[0])[0]-2, min(flipped, key=lambda x:x[1])[1]-1), (max(flipped, key=lambda x:x[0])[0]+2, max(flipped, key=lambda x:x[1])[1]+1)]
        return(s)
